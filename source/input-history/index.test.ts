...
describe("Input History", () => {
  ...
  it("should load existing data from database", async () => {
    await db()
      ...
      .values([{ input: "existing 1" }, { input: "existing 2" }]);
  ...
  it("should truncate old entries when history exceeds limit", async () => {
    ...
    const numCommandsToTest =
      _exportedForTest.MAX_HISTORY_ITEMS + _exportedForTest.MAX_HISTORY_TRUNCATION_BATCH;
    for (let i = 1; i <= numCommandsToTest; i++) {
      await inputHistory.appendToInputHistory(`command ${i}`);
    }

    const totalRows = await db().select({ count: count() }).from(inputHistoryTable);
