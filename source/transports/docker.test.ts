...
describe("DockerTransport", () => {
  it("reports container cleanup failures and shares concurrent cleanup requests", async () => {
    ...
    await withMock(
      ...
      async () => {
        ...
        await Promise.all([container.close(), container.close(), manager.terminateOnOctoExit()]);
  ...
  it("creates a process group when detached", async () => {
    const { transport, spawn } = await harness();
    transport.spawn("bash", ["-c", "printf hello"], { detached: true });

    expect(spawn).toHaveBeenCalledWith(
      "docker",
      ["exec", "-i", "sandbox", "bash", "-c", "printf hello"],
      expect.objectContaining({ detached: true }),
    );
  });
