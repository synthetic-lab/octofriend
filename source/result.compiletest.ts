import { err, flatten, Flattened, ok, Result } from "./result.ts";
...
const innerError: Result<Result<Result<number, "inner">, never>, never> = ok(ok(err("inner")));
const flattenedInnerError = flatten(innerError);
expectType<Result<number, "inner">>(flattenedInnerError);

const middleError: Result<Result<Result<number, never>, "middle">, never> = ok(err("middle"));
