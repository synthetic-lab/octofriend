...
import { ToolError, attempt, attemptUntrackedStat } from "./common.ts";
...
export async function listDir(toolCall: t.GetType<typeof ListToolSchema>) {
...
export async function validateListDir(toolCall: t.GetType<typeof ListToolSchema>) {
  const dirpath = toolCall?.params?.dirPath || process.cwd();
