use octofriend_tools::web::{SearchRequest, SearchResult, render_search_results};

#[test]
fn builds_search_post_requests_with_bearer_auth_and_json_body() {
    let request = SearchRequest::new("https://search.example/query", "search-key", "octofriend");

    assert_eq!(request.url, "https://search.example/query");
    assert_eq!(request.method, "POST");
    assert_eq!(request.authorization_header, "Bearer search-key");
    assert_eq!(request.body, r#"{"query":"octofriend"}"#);
}
