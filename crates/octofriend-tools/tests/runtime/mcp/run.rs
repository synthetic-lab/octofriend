#[cfg(not(windows))]
use octofriend_tools::runtime::run_runtime_tool_call;
#[cfg(not(windows))]
use serde_json::json;
...
fn runs_runtime_mcp_tool_calls_through_stdio_server() {
    let root = unique_temp_dir("octofriend-tools-mcp-run");
...
fn resolves_runtime_mcp_server_from_config_through_agentd() {
    let root = unique_temp_dir("octofriend-tools-mcp-config-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
    let server_path = root.join("fake-mcp-server.sh");
    std::fs::write(
        &server_path,
