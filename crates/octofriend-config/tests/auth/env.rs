...
use octofriend_config::auth::{AuthError, EnvAuth, KeyResult, resolve_env_auth_from};

#[test]
fn resolves_non_empty_env_values_and_reports_missing_or_empty_values() {
    let mut env = BTreeMap::new();
    env.insert("OPENAI_API_KEY".into(), " sk-test ".into());
    env.insert("EMPTY_KEY".into(), String::new());
    env.insert("WHITESPACE_KEY".into(), " \n\t ".into());
    ...
    assert_eq!(
        resolve_env_auth_from(&EnvAuth::new("WHITESPACE_KEY"), &env),
        KeyResult::Err {
            error: AuthError::Missing {
                message: "Environment variable WHITESPACE_KEY is not set".into()
            }
        }
    );
    assert_eq!(
