...
use octofwen_core::ids::ToolCallId;
use serde_json::json;
...
fn represents_schema_references_without_copying_inline_schema_text() {
    let declaration = ToolDeclaration::new(
        ...
        ToolSchemaReference::reference("octofwen://schemas/tools/apply-patch"),
    );

    assert_eq!(
        declaration.schema,
        ToolSchemaReference::Ref("octofwen://schemas/tools/apply-patch".into())
    );
}
