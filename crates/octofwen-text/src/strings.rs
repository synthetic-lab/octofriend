use octofwen_core::errors::{OctofwenError, OctofwenResult};
...
pub fn insert_at(value: &str, index: usize, insertion: &str) -> OctofwenResult<String> {
    ...
    if index >= char_count {
        return Err(OctofwenError::new("inserting past end of string"));
    }
...
pub fn cut_index(value: &str, index: usize) -> OctofwenResult<String> {
    let char_count = value.chars().count();
    if char_count == index + 1 {
    ...
    if index >= char_count {
        return Err(OctofwenError::new("cutting past end of string"));
    }
