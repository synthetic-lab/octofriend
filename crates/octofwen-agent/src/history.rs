use octofwen_store::record::conversation::ConversationHistoryKind;
use octofwen_store::repo::history::{ConversationHistoryOptions, ConversationHistoryRepository};
...
fn open_repository(
    database_path: Option<PathBuf>,
) -> octofwen_store::sqlite::connection::StorageResult<ConversationHistoryRepository> {
    ConversationHistoryRepository::open(ConversationHistoryOptions { database_path })
}

fn storage_error(
    id: JsonRpcId,
    error: octofwen_store::sqlite::connection::StorageError,
) -> JsonRpcResponse {
