...
fn tool_parse_input(
    provider: ToolCallParseProviderParam,
    tool: ToolCallParseToolParam,
) -> Result<octofwen_models::compiler::ToolParseInputItem, String> {
...
fn tool_parse_execution_result_json(request: &ToolParseExecutionRequest) -> Value {
    ...
    match result {
        octofwen_models::compiler::ToolParseExecutionResult::Success { tool } => json!({
            "status": "success",
            "tool": tool,
        }),
        octofwen_models::compiler::ToolParseExecutionResult::Error { message } => {
