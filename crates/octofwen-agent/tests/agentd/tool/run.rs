use octofwen_agent::runtime::{AGENTD_TOOL_RUN_METHOD, handle_agentd_json_rpc_line};
use serde_json::json;

#[test]
fn tool_run_request_returns_tool_run_result() {
    let root = unique_temp_dir("octofwen-agentd-run");
...
fn tool_run_request_runs_shell_in_docker_transport() {
    let root = unique_temp_dir("octofwen-agentd-docker-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
    ...
    let line = json!({
        ...
        "params": {
            "toolName": "shell",
            "cwd": root,
            "transport": { "type": "docker", "container": "octofwen-test-container" },
            "toolCallId": "shell-1",
            "toolCall": {
                "type": "tool-call",
                "toolCallId": "shell-1",
...
fn tool_run_request_reads_files_in_docker_transport() {
    let root = unique_temp_dir("octofwen-agentd-docker-read");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
    std::fs::write(
        root.join("read.txt"),
        "host
...
fn tool_run_request_runs_search_tools_in_docker_transport() {
    let root = unique_temp_dir("octofwen-agentd-docker-search");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
    let fake_docker = root.join("docker");
    std::fs::write(
        &fake_docker,
...
fn tool_run_request_returns_mcp_tool_result() {
    let root = unique_temp_dir("octofwen-agentd-mcp-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn tool_run_request_inserts_lsp_config_before_tool_run() {
    let root = unique_temp_dir("octofwen-agentd-lsp-run");
...
fn tool_run_request_inserts_skill_context_before_tool_run() {
    let root = unique_temp_dir("octofwen-agentd-skill-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn tool_run_request_inserts_web_search_config_before_tool_run() {
    ...
    let root = unique_temp_dir("octofwen-agentd-web-search-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn run_agentd_with_path(line: &str, path_dir: &std::path::Path) -> String {
    let binary = env!("CARGO_BIN_EXE_octofwen-agentd");
