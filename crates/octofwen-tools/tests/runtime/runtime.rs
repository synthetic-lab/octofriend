...
fn successful_shell_command() -> &'static str {
    "set /p OCTOFWEN_TEST=hello<NUL & exit /b 0"
}
...
fn failing_shell_command() -> &'static str {
    "set /p OCTOFWEN_TEST=nope<NUL & exit /b 7"
}
...
fn validates_runtime_file_and_workspace_tool_calls() {
    let root = unique_temp_dir("octofwen-tools-validate");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn runs_runtime_file_and_workspace_tool_calls() {
    let root = unique_temp_dir("octofwen-tools-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
    std::fs::create_dir_all(root.join("src")).expect("src dir should be created");
...
fn runs_runtime_shell_tool_calls() {
    let root = unique_temp_dir("octofwen-tools-shell-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn runs_runtime_skill_tool_calls() {
    let root = unique_temp_dir("octofwen-tools-skill-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn runs_runtime_fetch_tool_calls() {
    ...
    let root = unique_temp_dir("octofwen-tools-fetch-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
...
fn runs_runtime_web_search_tool_calls() {
    ...
    let root = unique_temp_dir("octofwen-tools-web-search-run");
    std::fs::create_dir_all(&root).expect("temp dir should be created");
    ...
    let output = run_runtime_tool_call(
        ...
        &root,
        "search-1",
        &json!({ "type": "tool-call", "toolCallId": "search-1", "name": "web-search", "original": { "query": "octofwen" }, "parsed": { "query": "octofwen" } }),
        &json!({ "query": "octofwen", "searchUrl": server.url(), "searchKey": "search-key" }),
    )
...
fn runs_runtime_grep_tool_calls() {
    let root = unique_temp_dir("octofwen-tools-grep-run");
...
fn runs_runtime_glob_tool_calls() {
    let root = unique_temp_dir("octofwen-tools-glob-run");
    std::fs::create_dir_all(root.join("src/nested")).expect("fixture dirs should be created");
