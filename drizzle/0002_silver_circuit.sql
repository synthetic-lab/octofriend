...
CREATE UNIQUE INDEX `tree_nodes_history_item_id_unique` ON `tree_nodes` (`history_item_id`);
--> statement-breakpoint
CREATE UNIQUE INDEX `tree_nodes_id_tree_id_unique` ON `tree_nodes` (`id`,`tree_id`);
--> statement-breakpoint
CREATE UNIQUE INDEX `tree_nodes_one_root_unique` ON `tree_nodes` (`tree_id`) WHERE `parent_id` IS NULL;
--> statement-breakpoint
CREATE INDEX `tree_nodes_tree_leaf_id_idx` ON `tree_nodes` (`tree_id`,`is_leaf`,`id`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX `trees_cwd_updated_at_idx` ON `trees` (`cwd`,`updated_at`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `history_items_request_failed_id_unique` ON `history_items` (`request_failed_id`);
--> statement-breakpoint
CREATE UNIQUE INDEX `history_items_compaction_failed_id_unique` ON `history_items` (`compaction_failed_id`);
--> statement-breakpoint
CREATE UNIQUE INDEX `history_items_notification_id_unique` ON `history_items` (`notification_id`);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `launches_docker_launch_id_unique` ON `launches` (`docker_launch_id`);
--> statement-breakpoint
CREATE UNIQUE INDEX `launches_local_launch_id_unique` ON `launches` (`local_launch_id`);
--> statement-breakpoint
CREATE TABLE `docker_launches` (
	...
	CONSTRAINT "docker_launches_kind_args_check" CHECK(("docker_launches"."kind" = 'connect' AND "docker_launches"."target" IS NOT NULL AND "docker_launches"."docker_run_args_json" IS NULL)
		OR ("docker_launches"."kind" = 'run' AND "docker_launches"."target" IS NULL AND "docker_launches"."docker_run_args_json" IS NOT NULL))
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX `shown_update_notifs_update_unique` ON `shown_update_notifs` (`update`);
